...
export function createPublisher({ repositoryRoot, store, validateCommand } = {}) {
  ...
  async function prepare(input) {
    const publishInput = {
      ...input,
      metadata: { ...input.metadata, status: 'published' },
    };
    ...
    return {
      ...
      remote: git.remote,
      files: changedFiles,
      suggestedMessage: `feat: xuất bản bài ${article.metadata.title}`.slice(0, 160),
    };
  }
  ...
  async function confirm({ token, message }) {
    ...
    plans.delete(token);
    if (!plan || plan.expiresAt < Date.now()) throw new ArticleStoreError('Phiên xác nhận đã hết hạn. Hãy kiểm tra lại trước khi xuất bản.', 409, 'PUBLISH_PLAN_EXPIRED');
    ...
    if (git.branch !== plan.branch || git.snapshot !== plan.snapshot) {
      throw new ArticleStoreError('Repository đã thay đổi sau bước kiểm tra. Hãy tạo lại danh sách xuất bản.', 409, 'PUBLISH_PLAN_CHANGED');
    }
    ...
    return {
      ...
      message: 'Đã commit và push. Cloudflare sẽ tự build từ GitHub.',
    };
  }
